...
class Navigation {
    public:
        ...
        int obstacleAvoidance(int currentPosition[], int targetPosition[]); // Avoid obstacles to navigate to target position
        ...
        int scanAndCalculateWidth(int position[]); // Get initial sensor readings and then scan left and right by moving OTV
        double calculateWidthFromMeasurements(const std::vector<SensorReadings>& measurements); // Calculate object width from scanned sensor readings
        double calculateObjectWidth(int leftSensor, int centerSensor, int rightSensor); // calculate indivual sensor reading width
        int sensorOne();
        int sensorTwo();
        int sensorThree();
        int initializeSensors(); // Initialize the sensors
